...
This document is the working standard for every upcoming TechMedia workspace module. Agents must use it before creating or changing list, show, upsert, lookup, select, date, status, table, and form experiences.
...
- Add `noValidate` to forms so browser-native validation does not replace the TechMedia banner pattern.
...
- Form tabs must use the shared TechMedia tone: transparent tab list, one bottom divider under the tab row, animated primary underline, and a small gap before the active tab content.
...
- [ ] No role file is an alias-only wrapper, empty scaffold, metadata-only placeholder, or â€œreserved for futureâ€ declaration.
