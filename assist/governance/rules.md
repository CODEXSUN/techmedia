...
- TECHMEDIA uses npm only. Run dependency commands from the current Git repository root; pnpm, Yarn, workspace-local installs, and alternative lockfiles are prohibited.
...
- TECHMEDIA is a modular monolith unless a stronger reason exists.
...
- Role files must not be one-line aliases, empty arrays, metadata-only placeholders, â€œreserved for futureâ€ declarations, or exports of another role under a new name. A list owns list rendering, a form owns form rendering and validation display, events own typed event construction, workers own executable job dispatch, sync owns an explicit sync policy and decision behavior, and seeds/migrations execute or expose callable database behavior.
