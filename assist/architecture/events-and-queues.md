...
TECHMEDIA should use events to keep modules independent while allowing them to react to important business moments.
...
TECHMEDIA should use Framework queue contracts so queue backends are switchable.
...
- Local/development backend is `database`, controlled by `TECHMEDIA_QUEUE_BACKEND=database`.
- `bullmq-redis` is connected through BullMQ and Redis using `TECHMEDIA_REDIS_URL`; database metadata is still retained so Queue Management can show status, filters, retries, and audit context.
