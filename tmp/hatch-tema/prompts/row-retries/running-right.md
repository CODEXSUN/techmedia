Create Codex pet row `running-right` for `tema`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Compact friendly purple 3D toy robot, glossy black screen face with cyan pixel eyes and smile, single antenna, cyan chest power light, purple vest, sturdy sneakers, approachable business helper. Preserve identity but omit readable words and tiny logos at sprite size.. Preserve silhouette, face, palette, material, proportions, markings, and props.
