...
export class TenantUserService {
  ...
  private async required(id: string): Promise<TenantUser> {
  ...
  private async credentialsForSave(
    ...
    current: Awaited<ReturnType<TenantUserRepository["findFrappeCredentials"]>>
  ) {
...
export function tenantUserFrappeCredentialContract(database: Kysely<TenantDatabase>) {
  const repository = new TenantUserRepository(database);
  return {
...
/**
 * Public application-service contract used by Frappe user sync. Tenant User
 * remains the only module that creates local user rows and hashes passwords.
 */
export function tenantUserFrappeImportContract(context: {
  actorEmail: string;
  database: Kysely<TenantDatabase>;
  tenantId: string;
}) {
  const repository = new TenantUserRepository(context.database);
  return {
    async importUser(input: TenantUserFrappeImportPayload): Promise<TenantUserFrappeImportResult> {
