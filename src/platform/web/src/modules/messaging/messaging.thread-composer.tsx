...
export function ThreadComposer({ onSend }: { onSend: (content: string, type: MessageType, metadata: Record<string, unknown>) => Promise<boolean> }) {
  ...
  useEffect(() => {
    const reply = (event: Event) => {
      ...
      setDraft((current) => `> ${message.senderName}: ${message.content}\n${current}`);
  ...
  const pickFile = async (file?: File) => {
    if (!file) return;
    if (file.size > 600 * 1024) { setError("Files must be 600 KB or smaller."); return; }
    setError("");
  ...
  const submit = async () => {
    ...
    if (await onSend(isTask ? content.slice(6).trim() : content, isTask ? "TASK" : attachment?.kind ?? "TEXT", metadata)) { setDraft(""); setSelectedMentions([]); setAttachment(null); setError(""); }
  };
