...
const privateKeyPath = join(directory, "github_techmedia");
const publicKeyPath = `${privateKeyPath}.pub`;
const comment = options.comment ?? `techmedia-server@${hostname()}`;
...
function printHelp() {
  output.log(`Generate a temporary ED25519 keypair for TECHMEDIA server access to GitHub.

Usage:
  npm run github:ssh-key
  npm run github:ssh-key -- --comment "techmedia-server"
